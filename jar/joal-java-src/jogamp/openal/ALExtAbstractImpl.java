...
/* !---- Java-Unit: [pkg jogamp.openal, cls ALExtAbstractImpl], ../build/gensrc/classes/jogamp/openal/ALExtAbstractImpl.java ----! */
...
public abstract class ALExtAbstractImpl implements ALExt{
  ...
  public void alEventControlSOFT(int count, IntBuffer types, boolean enable)  {
    ...
    final long __addr_ = alExtProcAddressTable._addressof_alEventControlSOFT;
    if (__addr_ == 0) {
      throw new UnsupportedOperationException(String.format("Method \"%s\" not available", "alEventControlSOFT"));
    }
    dispatch_alEventControlSOFT1(count, types_is_direct ? types : Buffers.getArray(types), types_is_direct ? Buffers.getDirectBufferByteOffset(types) : Buffers.getIndirectBufferByteOffset(types), types_is_direct, enable, __addr_);
  }

  /** Entry point (through function pointer) to C language function: <br> <code>void alEventControlSOFT(ALsizei count, const ALenum *  types, ALboolean enable)</code><br>
      @param types a direct or array-backed {@link java.nio.IntBuffer}   */
  private native void dispatch_alEventControlSOFT1(int count, Object types, int types_byte_offset, boolean types_is_direct, boolean enable, long procAddress);
  ...
  public void alEventControlSOFT(int count, int[] types, int types_offset, boolean enable)  {
    ...
    if(types != null && types.length <= types_offset)
      throw new ALException("array offset argument \"types_offset\" (" + types_offset + ") equals or exceeds array length (" + types.length + ")");
    final long __addr_ = alExtProcAddressTable._addressof_alEventControlSOFT;
    if (__addr_ == 0) {
      throw new UnsupportedOperationException(String.format("Method \"%s\" not available", "alEventControlSOFT"));
    }
    dispatch_alEventControlSOFT1(count, types, Buffers.SIZEOF_INT * types_offset, false, enable, __addr_);
  }
  ...
  public final Set<ALContextKey> getAlEventCallbackSOFTKeys() {
    synchronized( alEventCallbackSOFTLock ) {
      return alEventCallbackSOFTDataMap.keySet();
    }
  }
  ...
  public final boolean isAlEventCallbackSOFTMapped(ALContextKey key) {
    synchronized( alEventCallbackSOFTLock ) {
      return null != alEventCallbackSOFTDataMap.get(key);
    }
  }
  ...
  public final ALEVENTPROCSOFT getAlEventCallbackSOFT(ALContextKey key) {
    synchronized( alEventCallbackSOFTLock ) {
      final AlEventCallbackSOFTData value = alEventCallbackSOFTDataMap.get(key);
  ...
  public final int releaseAllAlEventCallbackSOFT() {
    synchronized( alEventCallbackSOFTLock ) {
      final Set<ALContextKey> keySet = alEventCallbackSOFTDataMap.keySet();
      final ALContextKey[] keys = keySet.toArray(new ALContextKey[keySet.size()]);
      for(int i=0; i<keys.length; ++i) {
        final ALContextKey key = keys[i];
          releaseAlEventCallbackSOFT(key);
      }
      return keys.length;
    }
  }
  ...
  public final void releaseAlEventCallbackSOFT(ALContextKey key) {
    synchronized( alEventCallbackSOFTLock ) {
      AlEventCallbackSOFTData value = alEventCallbackSOFTDataMap.remove(key);
      if( null != value ) {
        alEventCallbackSOFTUserObjIDMap.remove(value.paramID);
      }
    }
  }

  private final void addAlEventCallbackSOFT(ALCcontext userParam, AlEventCallbackSOFTData value) {
    final ALContextKey key = new ALContextKey(userParam);
    final AlEventCallbackSOFTData old = alEventCallbackSOFTDataMap.put(key, value);
    if( null != old ) {
      alEventCallbackSOFTUserObjIDMap.remove(old.paramID);
    }
    if( null != value.param ) {
      alEventCallbackSOFTUserObjIDMap.put(value.paramID, value.param);
    }
  }

  private static class AlEventCallbackSOFTData {
    final ALEVENTPROCSOFT func;
    // userParamArgCType void * 
    // userParamArgJType ALCcontext
    final ALCcontext param;
    final long paramID;
    AlEventCallbackSOFTData(final ALEVENTPROCSOFT func, final long paramID, final ALCcontext param) {
      this.func = func;
      this.paramID = paramID;
      this.param = param;
    }
  }
  private static final Map<ALContextKey, AlEventCallbackSOFTData> alEventCallbackSOFTDataMap = new HashMap<ALContextKey, AlEventCallbackSOFTData>();
  private static final LongObjectHashMap alEventCallbackSOFTUserObjIDMap = new LongObjectHashMap();
  private static long NEXT_AlEventCallbackSOFT_ID = 1;
  private static final Object alEventCallbackSOFTLock = new Object();

  /** Static callback invocation, dispatching to ALEVENTPROCSOFT for callback <br> <code>void (*ALEVENTPROCSOFT)(ALenum eventType, ALuint object, ALuint param, ALsizei length, const ALchar *  message, void *  userParam)</code> */
  /* pp */ static void invokeAlEventCallbackSOFT(int eventType, int object, int param, int length, java.lang.String message, long nativeUserParam) {
    final ALCcontext userParam;
    final AlEventCallbackSOFTData value;
    synchronized( alEventCallbackSOFTLock ) {
      userParam = (ALCcontext) alEventCallbackSOFTUserObjIDMap.get(nativeUserParam);
      final ALContextKey key = new ALContextKey(userParam);
      value = alEventCallbackSOFTDataMap.get(key);
    }
    if( null == value ) {
      return;
    }
    value.func.callback(eventType, object, param, length, message, userParam);
  }
  ...
  public void alEventCallbackSOFT(ALEVENTPROCSOFT callback, ALCcontext userParam)  {
    ...
    final long __addr_ = alExtProcAddressTable._addressof_alEventCallbackSOFT;
    if (__addr_ == 0) {
      throw new UnsupportedOperationException(String.format("Method \"%s\" not available", "alEventCallbackSOFT"));
    }
    final long nativeUserParam;
    synchronized( alEventCallbackSOFTLock ) {
      if( null != userParam ) {
        nativeUserParam = NEXT_AlEventCallbackSOFT_ID++;
        if( 0 >= NEXT_AlEventCallbackSOFT_ID ) { NEXT_AlEventCallbackSOFT_ID = 1; }
      } else {
        nativeUserParam = 0;
      }
      if( null != callback ) {
        addAlEventCallbackSOFT(userParam, new AlEventCallbackSOFTData(callback, nativeUserParam, userParam));
      } else { 
        // release a previously mapped instance 
        releaseAlEventCallbackSOFT( new ALContextKey( userParam ) );
      }
    } // synchronized 

    dispatch_alEventCallbackSOFT1(callback, userParam, this.getClass(), "(IIIILjava/lang/String;J)V", nativeUserParam, __addr_);
  }

  /** Entry point (through function pointer) to C language function: <br> <code>void alEventCallbackSOFT(ALEVENTPROCSOFT callback, void *  userParam)</code><br>   */
  private native void dispatch_alEventCallbackSOFT1(ALEVENTPROCSOFT callback, ALCcontext userParam, Class<?> staticCBClazz, String callbackSignature, long nativeUserParam, long procAddress);
  ...
  public ByteBuffer alGetPointerSOFT(int pname)  {
    ...
    final long __addr_ = alExtProcAddressTable._addressof_alGetPointerSOFT;
    if (__addr_ == 0) {
      throw new UnsupportedOperationException(String.format("Method \"%s\" not available", "alGetPointerSOFT"));
    }
    final ByteBuffer _res;
    _res = dispatch_alGetPointerSOFT1(pname, __addr_);
    if (_res == null) return null;
  ...
  /** Entry point (through function pointer) to C language function: <br> <code>void *  alGetPointerSOFT(ALenum pname)</code><br>   */
  private native ByteBuffer dispatch_alGetPointerSOFT1(int pname, long procAddress);
  ...
  public void alGetPointervSOFT(int pname, PointerBuffer values)  {
    ...
    final boolean values_is_direct = Buffers.isDirect(values);
    final long __addr_ = alExtProcAddressTable._addressof_alGetPointervSOFT;
    if (__addr_ == 0) {
      throw new UnsupportedOperationException(String.format("Method \"%s\" not available", "alGetPointervSOFT"));
    }
    dispatch_alGetPointervSOFT1(pname, values_is_direct ? ( values != null ? values.getBuffer() : null ) : Buffers.getArray(values), values_is_direct ? Buffers.getDirectBufferByteOffset(values) : Buffers.getIndirectBufferByteOffset(values), values_is_direct, __addr_);
  }

  /** Entry point (through function pointer) to C language function: <br> <code>void alGetPointervSOFT(ALenum pname, void *  *  values)</code><br>
      @param values a direct or array-backed {@link com.jogamp.common.nio.PointerBuffer}   */
  private native void dispatch_alGetPointervSOFT1(int pname, Object values, int values_byte_offset, boolean values_is_direct, long procAddress);
