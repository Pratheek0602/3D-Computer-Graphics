...
public class Mesh {
    ...
    public Mesh(GL3 gl, float[] vertices, int[] indices, int[] attributeSizes) {
        this.vertices = vertices;
        this.indices = indices;
        this.attributeSizes = attributeSizes;
        this.vertexStride = 0;
        for (int size : attributeSizes) {
            this.vertexStride += size;
        }
        fillBuffers(gl);
    }
    ...
    private void fillBuffers(GL3 gl) {
        ...
        for (int i = 0; i < attributeSizes.length; i++) {
            gl.glVertexAttribPointer(i, attributeSizes[i], GL.GL_FLOAT, false, vertexStride * Float.BYTES, offset * Float.BYTES);
